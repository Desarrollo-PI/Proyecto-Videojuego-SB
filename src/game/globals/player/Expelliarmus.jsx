...
export default function Expelliarmus() {
  ...
  useFrame((state, delta) => {
    expelliarmusBodyRef?.current?.setLinvel(
      {
        x: player.expelliarmusInitRotation.x * 10,
        y: 0,
        z: player.expelliarmusInitRotation.z * 10,
      },
      true
    )
    const desplazamiento = {
      x: Math.abs(
        expelliarmusBodyRef?.current?.translation().x -
          player.expelliarmusInitPosition[0]
      ),
      z: Math.abs(
        expelliarmusBodyRef?.current?.translation().z -
          player.expelliarmusInitPosition[2]
      ),
    }
  ...
  return (
    <RigidBody
      ...
      position={player.expelliarmusInitPosition}
