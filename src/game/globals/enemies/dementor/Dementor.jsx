...
export function Dementor(props) {
	...
	const { nodes, materials, animations } = useGLTF('/assets/models/characters/enemies/Dementor.glb')
	...
	useFrame(({ clock }, delta) => {
		if (props.action == 1) {
			dementorRef.current.position.x = Math.sin(clock.getElapsedTime()) + props.position[0]
		}
		dementorRef.current.position.y = Math.sin(clock.getElapsedTime() * 2) / 5 + props.position[1]
	})
