...
export function EvilWizard(props) {
	...
	const { nodes, materials, animations } = useGLTF('/assets/models/characters/enemies/EvilWizard.glb')
	...
	useFrame(({ clock }, delta) => {
		if (props.action == 1) {
			if (evilWizardRef.current.position.z < Math.sin(clock.getElapsedTime()) * 3 + props.position[2]) {
				evilWizardRef.current.rotation.y = 0
			} else {
				evilWizardRef.current.rotation.y = Math.PI
			}
			evilWizardRef.current.position.z = Math.sin(clock.getElapsedTime()) * 3 + props.position[2]
		}
	})

	return (
		<group ref={evilWizardRef} {...props} dispose={null} position={props.position} scale={0.01}>
			<group name="Scene">
