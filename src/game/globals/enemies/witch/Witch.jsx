...
export function Witch(props) {
	...
	const { nodes, materials, animations } = useGLTF('/assets/models/characters/enemies/Witch.glb')
	...
	useFrame(({ clock }, delta) => {
		if (props.action == 1) {
			if (witchRef.current.position.z < Math.sin(clock.getElapsedTime()) * 3 + props.position[2]) {
				witchRef.current.rotation.y = 0
			} else {
				witchRef.current.rotation.y = Math.PI
			}
			witchRef.current.position.z = Math.sin(clock.getElapsedTime()) * 3 + props.position[2]
		}
	})

	return (
		<group ref={witchRef} {...props} dispose={null}  position={props.position} scale={0.25}>
			<group name="Scene">
